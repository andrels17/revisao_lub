...
def calcular_proximas_lubrificacoes_batch(equipamento_ids):
    ...
    try:
        ...
        km_base_expr = "coalesce(e.km_base_plano, e.km_atual, 0)" if "km_base_plano" in colunas else "coalesce(e.km_atual, 0)"
        horas_base_expr = "coalesce(e.horas_base_plano, e.horas_atual, 0)" if "horas_base_plano" in colunas else "coalesce(e.horas_atual, 0)"
        cur.execute(
            f"""
            select
                ...
                e.km_atual,
                e.horas_atual,
                {km_base_expr} as km_base_plano,
                {horas_base_expr} as horas_base_plano,
                tl.tipo_controle,
                itl.id as item_id,
                itl.nome_item,
                itl.tipo_produto,
        ...
        for eqp_id, km_atual, horas_atual, km_base_plano, horas_base_plano, tipo_controle, item_id, nome_item, tipo_produto, intervalo in rows:
            leitura_atual = float(horas_atual if tipo_controle == "horas" else km_atual)
            leitura_base = float(horas_base_plano if tipo_controle == "horas" else km_base_plano)
            ...
            resultado[eqp_id].append({
                ...
                "km_base_plano":     float(km_base_plano or 0),
                "horas_base_plano":  float(horas_base_plano or 0),
