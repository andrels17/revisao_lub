from database.connection import get_conn


def listar():
    conn = get_conn()
    cur = conn.cursor()
    try:
        cur.execute(
            """
            select
                e.id,
                e.codigo,
                e.nome,
                e.tipo,
                e.km_atual,
                e.horas_atual,
                e.template_revisao_id,
                e.setor_id,
                coalesce(s.nome, '-') as setor_nome,
                e.template_lubrificacao_id
            from equipamentos e
            left join setores s on s.id = e.setor_id
            order by e.codigo
            """
        )
        rows = cur.fetchall()
        return [
            {
                "id": r[0],
                "codigo": r[1],
                "nome": r[2],
                "tipo": r[3],
                "km_atual": float(r[4] or 0),
                "horas_atual": float(r[5] or 0),
                "template_revisao_id": r[6],
                "setor_id": r[7],
                "setor_nome": r[8],
                "template_lubrificacao_id": r[9],
            }
            for r in rows
        ]
    finally:
        conn.close()


def criar(codigo, nome, tipo, setor_id, km_atual=0, horas_atual=0,
          template_revisao_id=None, ativo=True):
    """Mantido para compatibilidade com importação CSV e código existente."""
    return criar_completo(
        codigo=codigo, nome=nome, tipo=tipo, setor_id=setor_id,
        km_atual=km_atual, horas_atual=horas_atual,
        template_revisao_id=template_revisao_id, ativo=ativo,
    )


def criar_completo(codigo, nome, tipo, setor_id, km_atual=0, horas_atual=0,
                   template_revisao_id=None, template_lubrificacao_id=None, ativo=True):
    conn = get_conn()
    cur = conn.cursor()
    try:
        cur.execute(
            """
            insert into equipamentos (
                codigo, nome, tipo, setor_id,
                km_atual, horas_atual,
                template_revisao_id, template_lubrificacao_id, ativo
            )
            values (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            returning id
            """,
            (
                codigo, nome, tipo, setor_id,
                km_atual, horas_atual,
                template_revisao_id, template_lubrificacao_id, ativo,
            ),
        )
        equipamento_id = cur.fetchone()[0]
        conn.commit()
        return equipamento_id
    finally:
        conn.close()



def obter_por_id(equipamento_id):
    conn = get_conn()
    cur = conn.cursor()
    try:
        cur.execute(
            """
            select
                e.id,
                e.codigo,
                e.nome,
                e.tipo,
                e.km_atual,
                e.horas_atual,
                e.template_revisao_id,
                e.setor_id,
                coalesce(s.nome, '-') as setor_nome,
                e.template_lubrificacao_id
            from equipamentos e
            left join setores s on s.id = e.setor_id
            where e.id = %s
            """,
            (equipamento_id,),
        )
        r = cur.fetchone()
        if not r:
            return None
        return {
            "id": r[0],
            "codigo": r[1],
            "nome": r[2],
            "tipo": r[3],
            "km_atual": float(r[4] or 0),
            "horas_atual": float(r[5] or 0),
            "template_revisao_id": r[6],
            "setor_id": r[7],
            "setor_nome": r[8],
            "template_lubrificacao_id": r[9],
        }
    finally:
        conn.close()


def buscar_rapida(termo, limite=10):
    termo = (termo or '').strip()
    if not termo:
        return []

    conn = get_conn()
    cur = conn.cursor()
    try:
        like = f"%{termo}%"
        cur.execute(
            """
            select
                e.id,
                e.codigo,
                e.nome,
                e.tipo,
                e.km_atual,
                e.horas_atual,
                e.template_revisao_id,
                e.setor_id,
                coalesce(s.nome, '-') as setor_nome,
                e.template_lubrificacao_id
            from equipamentos e
            left join setores s on s.id = e.setor_id
            where (
                e.codigo ilike %s
                or e.nome ilike %s
                or coalesce(e.tipo, '') ilike %s
                or coalesce(s.nome, '') ilike %s
            )
            order by
                case when e.codigo ilike %s then 0 else 1 end,
                case when e.nome ilike %s then 0 else 1 end,
                e.codigo
            limit %s
            """,
            (like, like, like, like, like, like, limite),
        )
        rows = cur.fetchall()
        resultados = []
        for r in rows:
            item = {
                "id": r[0],
                "codigo": r[1],
                "nome": r[2],
                "tipo": r[3],
                "km_atual": float(r[4] or 0),
                "horas_atual": float(r[5] or 0),
                "template_revisao_id": r[6],
                "setor_id": r[7],
                "setor_nome": r[8],
                "template_lubrificacao_id": r[9],
                "status_resumo": 'EM DIA',
            }
            resultados.append(item)

        # Enriquecimento leve de status para priorizar alertas sem travar a tela.
        try:
            from services import revisoes_service, lubrificacoes_service

            for item in resultados:
                status = 'EM DIA'
                try:
                    revisoes = revisoes_service.calcular_proximas_revisoes(item['id'])
                except Exception:
                    revisoes = []
                try:
                    lubs = lubrificacoes_service.calcular_proximas_lubrificacoes(item['id'])
                except Exception:
                    lubs = []

                todos = [x.get('status') for x in revisoes + lubs if x.get('status')]
                if 'VENCIDO' in todos:
                    status = 'VENCIDO'
                elif 'PROXIMO' in todos:
                    status = 'PROXIMO'
                item['status_resumo'] = status

            prioridade = {'VENCIDO': 0, 'PROXIMO': 1, 'EM DIA': 2}
            resultados.sort(key=lambda x: (prioridade.get(x.get('status_resumo'), 9), x.get('codigo') or ''))
        except Exception:
            pass

        return resultados
    finally:
        conn.close()
