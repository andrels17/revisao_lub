...
def render():
    ...
    with tab_rev:
        ...
        if df_rev.empty:
            ...
            with col_exp:
                botao_exportar_excel(_formatar_datas_exibicao(df_rev), "relatorio_revisoes", label="⬇️ Excel", key="exp_rel_rev")
            st.dataframe(_formatar_datas_exibicao(df_rev.drop(columns=["ID"])), use_container_width=True, hide_index=True)
    ...
    with tab_lub:
        ...
        if df_lub.empty:
            ...
            col_exp = st.columns([5, 1])[1]
            with col_exp:
                botao_exportar_excel(_formatar_datas_exibicao(df_lub), "relatorio_lubrificacoes", label="⬇️ Excel", key="exp_rel_lub")
            st.dataframe(_formatar_datas_exibicao(df_lub.drop(columns=["ID"])), use_container_width=True, hide_index=True)
