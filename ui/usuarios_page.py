...
def _badge_role(role: str) -> str:
    cores = {"admin": "🔴", "gestor": "🟠", "operador": "🔵", "visualizador": "⚪"}
...
def _form_criar():
    ...
    responsaveis = [None] + [r for r in responsaveis_service.listar() if r.get("ativo")]
    with st.form("form_criar_usuario", clear_on_submit=True):
        ...
        with col2:
            role = st.selectbox("Role *", list(auth_service.ROLE_LABELS.keys()), format_func=lambda r: auth_service.ROLE_LABELS[r])
            resp = st.selectbox(
                "Vincular a responsável",
                responsaveis,
                format_func=lambda r: "— nenhum —" if r is None else f"{r['nome']} ({r.get('funcao_principal') or 'sem função'})",
    ...
    if submitted:
        ...
        if not email:
            erros.append("E-mail obrigatório.")
        if not senha:
...
def _form_editar(usuario: dict):
    ...
    idx_resp = next((i for i, r in enumerate(responsaveis) if r and r["id"] == resp_atual_id), 0)
    ...
    with st.form(f"form_editar_{usuario['id']}", clear_on_submit=False):
        ...
        with col2:
            ativo = st.checkbox("Ativo", value=usuario["ativo"], disabled=(usuario["id"] == uid_logado))
            resp = st.selectbox(
        ...
        excluir = col_excluir.form_submit_button("Excluir", disabled=(usuario["id"] == uid_logado), use_container_width=True)

    if salvar:
        ok, msg = auth_service.editar_usuario(
            uid=usuario["id"], nome=nome, role=role, ativo=ativo,
            responsavel_id=resp["id"] if resp else None, nova_senha=nova_senha or None,
        )
        if ok:
            st.success(msg)
            st.rerun()
...
def render():
    ...
    render_page_intro(
        "Usuários do sistema",
        "Gerencie acessos, permissões e vínculos com responsáveis em uma interface mais organizada.",
        "Ferramentas",
    )
    ...
    with tab_lista:
        if not usuarios:
            ...
            for u in usuarios:
                ...
                with st.expander(f"{_badge_role(u['role'])} | {u['nome']} — {u['email']}{sufixo} | {status}"):
                    st.caption(
                        f"Responsável vinculado: **{u.get('responsavel_nome') or '—'}** | Último login: **{u.get('ultimo_login') or 'nunca'}**"
                    )
                    _form_editar(u)
    with tab_novo:
        _form_criar()
